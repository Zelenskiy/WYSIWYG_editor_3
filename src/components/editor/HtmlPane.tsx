...
function format(raw: string): string {
  ...
  for (const t of tokens) {
    ...
    const openMatch = /^<\s*([\w-]+)/.exec(t);
    const tagName = openMatch ? openMatch[1].toLowerCase() : "";
    const isVoid = VOIDS.has(tagName) || /\/\s*>$/.test(t);
    const isOpen = openMatch !== null && !isClose && !isVoid;

    // Token contains both <tag> and </tag> inline (e.g. <td>text</td>)
    const closeMatch = /<\/([\w-]+)\s*>$/.exec(t);
    const isSelfContained =
      isOpen &&
      closeMatch !== null &&
      closeMatch[1].toLowerCase() === tagName;

    if (isClose) depth = Math.max(0, depth - 1);
    out.push(INDENT.repeat(depth) + t);
    if (isOpen && !isSelfContained) depth++;
  }
  return out.join("\n");
}
    ...
    <div className="html-pane-wrap">
      <div className="html-pane-toolbar">
        <button
          ...
          title="Ctrl+Shift+F — форматувати"
        >
          ⌘ Форматувати
        </button>
        <span className="hp-hint">
          Tab = відступ · Ctrl+Shift+F = форматувати
        </span>
      </div>
